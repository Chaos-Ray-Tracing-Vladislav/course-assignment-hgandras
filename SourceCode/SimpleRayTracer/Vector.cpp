...
//Scalar operations
Vector2 Vector2::operator*(float a) const { return Vector2(a * x, a * y); }
...
Vector2 Vector2::norm() { return Vector2(x, y) / length(); }

Vector2 Vector2::zero() { return Vector2(0, 0); }


Vector2 operator*(float a, Vector2& vec) { return Vector2(a * vec.x, a * vec.y); }
float Dot(const Vector2& a, const Vector2& b) { return a.x * b.x + a.y * b.y; }
Vector2 Reflect(Vector2& incoming, Vector2& n) { return incoming - 2 * Dot(incoming, n) * n; }
...
Vector3::Vector3(Vector2 vec) : x(vec.x), y(vec.y), z(1){}

//Scalar operations
Vector3 Vector3::operator*(float a) const { return Vector3(a * x, a * y, a * z); }
...
Vector3 Vector3::norm() { return Vector3(x, y, z) / length(); }
...
Vector3 operator*(const float a, const Vector3& vec) { return Vector3(a * vec.x, a * vec.y, a * vec.z); }
...
Vector4::Vector4(Vector3 vec) : x(vec.x), y(vec.y), z(vec.z), w(1){}

//Scalar operations
Vector4 Vector4::operator*(float a) const { return Vector4(a * x, a * y, a * z, a * w); }
Vector4 Vector4::operator/(float a) const { return Vector4(x / a, y / a, z / a, a / w); }
...
Vector4 Vector4::norm() { return Vector4(x, y, z, w) / length(); }

Vector4 Vector4::zero() { return Vector4(0, 0, 0, 0); }

Vector3 Vector4::xyz() { return Vector3(x, y, z); }

Vector4 operator*(float a, Vector4& vec) { return Vector4(a * vec.x, a * vec.y, a * vec.z, a * vec.w); }
...
Matrix3::Matrix3(){ data.insert(data.begin(), 9, 0); }

Matrix3::Matrix3(std::vector<float>& data) : data(data){}
...
Vector3 Matrix3::row1() { return Vector3(data[0], data[1], data[2]); }
Vector3 Matrix3::row2() { return Vector3(data[3], data[4], data[5]); }
Vector3 Matrix3::row3() { return Vector3(data[6], data[7], data[8]); }

Vector3 Matrix3::col1() { return Vector3(data[0], data[3], data[6]); }
Vector3 Matrix3::col2() { return Vector3(data[1], data[4], data[7]); }
Vector3 Matrix3::col3() { return Vector3(data[2], data[5], data[8]); }

Matrix3 Matrix3::T() { return Matrix3(row1(), row2(), row3());}

//Rotation in radians
Matrix3 Matrix3::CreateRotation(float x, float y, float z)
{
	//X rotation
...
Matrix3 Matrix3::operator*(float a){ return Matrix3(col1() * a, col2() * a, col3() * a); }

//Matrix4 implementations

Matrix4::Matrix4(){ data.insert(data.begin(), 16, 0); }
...
Matrix4::Matrix4(Matrix3 mat,Vector3 vec) : Matrix4(mat)
{
	data[3] = vec.x;
	data[7] = vec.y;
	data[11] = vec.z;
...
Vector4 Matrix4::row1() { return Vector4(data[0], data[1], data[2], data[3]); }
Vector4 Matrix4::row2() { return Vector4(data[4], data[5], data[6], data[7]); }
Vector4 Matrix4::row3() { return Vector4(data[8], data[9], data[10], data[11]); }
Vector4 Matrix4::row4() { return Vector4(data[12], data[13], data[14], data[15]); }

Vector4 Matrix4::col1() { return Vector4(data[0], data[4], data[8], data[12]); }
Vector4 Matrix4::col2() { return Vector4(data[1], data[5], data[9], data[13]); }
Vector4 Matrix4::col3() { return Vector4(data[2], data[6], data[10], data[14]); }
Vector4 Matrix4::col4() { return Vector4(data[3], data[7], data[11], data[15]); }

Matrix4 Matrix4::CreateTranslation(Vector3 delta){
	Matrix3 id = Matrix3::Identity();
	return Matrix4(id, delta); 
}

Matrix4 Matrix4::CreateRotation(float x, float y, float z)
{
...
Matrix4 Matrix4::T() { return Matrix4(row1(), row2(), row3(), row4()); }
